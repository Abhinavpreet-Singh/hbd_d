...
const lightInstruction = document.getElementById("lightInstruction");
...
lightsButton.addEventListener("click", function () {
  ...
  fadeOut(lightInstruction, 50);
  setTimeout(() => {
    speakerInstruction.style.display = "block";
  }, 500);
  // }
});
  ...
  const timer = setInterval(function () {
    ...
    if (getCenterPos(decorations) >= screenWidthCenterPos) {
      ...
      setTimeout(function () {
        ...
        okToClick = true;
        candleInstruction.style.display = "block";
      }, 5000);
    } else {
...
// Text navigation with next/back arrows
...
function goToNextText() {
  if (isTextPlaying && currentTextIndex < msgText.length - 1) {
    ...
    // Show next text after fade completes
    currentTextIndex++;
    setTimeout(() => {
...
function goToPreviousText() {
  if (isTextPlaying && currentTextIndex > 0) {
    ...
    // Show previous text after fade completes
    currentTextIndex--;
    setTimeout(() => {
      fadeIn(msgText[currentTextIndex], 20, "block");
      updateArrowVisibility();
